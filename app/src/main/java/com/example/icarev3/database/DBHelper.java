package com.example.icarev3.database;
...
public class DBHelper extends SQLiteOpenHelper {
        ...
        try {
            ...
            db.execSQL(
                    "create table if not exists profile " +
                            "(profile_id integer primary key, name text," +
                            " mobile text, relation text, birth_date text, " +
                            "height text, weight text, blood_group text, gender text, " +
                            "email text,diabetes text,blood_suger text,high_pressure text," +
                            "high_pressure_desc text,allergies text,allergic_description text," +
                            "suger_prev text,high_pressure_prev text,weight_prev text," +
                            "height_prev text)"
            );
            db.execSQL(
                    "create table if not exists doctor " +
                            "(doctor_id integer primary key, name text," +
                            " mobile text, email text, address text,user_id text)"
            );
            db.execSQL(
                    "create table if not exists health_center " +
                            "(health_center_id integer primary key, name text, " +
                            "mobile text, email text, address text, user_id text)"
            );
            db.execSQL(
                    "create table if not exists vaccine " +
                            "(vaccine_id integer primary key, title text, description text," +
                            " user_id text)"
            );
            db.execSQL(
                    "create table if not exists vaccine_dose " +
                            "(dose_id integer primary key, date text, dose_title text, " +
                            "vaccine_id text)"
            );
            db.execSQL(
                    "create table if not exists appointment " +
                            "(appointment_id integer primary key, title text, " +
                            "doctor_name text, care_center text, user_id text, date text, " +
                            "time text, alarm_date text, alarm_time text)"
            );

            db.execSQL(
                    "create table if not exists diet " +
                            "(diet_id integer primary key, date text, breakfast_menu text, " +
                            "lunch_menu text, diner_menu text, user_id text)"
            );
            db.execSQL(
                    "create table if not exists note " +
                            "(note_id integer primary key, date text, time text, title text," +
    ...
    public boolean insertMedicalCenter(String name, String mobile,
    ...
    public boolean insertAppointment(String title, String doctorName,
                                     String careCenterAddress, String date,
                                     String time, Integer id) {
    ...
    public boolean updateAppointment(String reminderDate, String reminderTime, Integer id) {

        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("alarm_date", reminderDate);
        contentValues.put("alarm_time", reminderTime);

        db.update("appointment", contentValues, "appointment_id = ? ", new String[]{Integer.toString(id)});
        return true;
    }
